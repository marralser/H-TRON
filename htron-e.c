/**************************************************************
*    htron.c                                   	              *
*    H-Tron - a DOS Tron game for EGA 640 x 350  *
*    written in Borland Turbo C 2.01                          *
*    Author: RobertK (RobertK@psc.at), December 2016          *
*    EGA conversion  and Amstrad Joystick by M Ralser 2026    *
*                                                             *
*    Controls:                                                *
...
/* EGAHI is 640 x 350. With 8 x 6 pixel blocks:
     80 columns = 640 pixels
     58 rows    = 348 pixels
   leaving two scan lines outside the logical grid. */
...
/* Playfield array that holds the information whether
   ...
   VGA playfield grid dimensions: 80 x 80
   x=0 to 79
   y=0 to 79
...
void InitPlayfield()
{
  ...
  /* Draw border */
  setcolor(WALL_COLOR);
  rectangle(0,0,639,349);  /* outer border */
  rectangle(7,6,632,18);   /* score area */
  rectangle(7,23,632,347); /* inner border (playfield) */
  ...
  */
  ...
  setcolor(P2_COLOR);
  outtextxy(610-textwidth(textP2),9,textP2);
  setcolor(SCORE_COLOR);
  outtextxy(320-(textwidth(textScore)/2),9,textScore);
...
void main()
{
  ...
  clrscr();
  printf("*** H-Tron ***\n");
  printf("A DOS game for a EGA graphics card (640 x 350),\n");
  printf("written 2016 by RobertK (RobertK@psc.at), extended 2026 by M Ralser.\n\n");
  ...
  /* Force the Borland BGI VGA driver and its 640 x 350 mode.
